...
### 5.1 Filter and Aggregate on Join Results using SQL
```sql
-- Task: Join the Orders and Customers tables to find the total order amount per customer 
-- and filter out customers who have spent less than $1,000.
SELECT 
    c.CustomerID,
    c.FirstName,
    c.LastName,
...
### 5.2 Cumulative Aggregations and Ranking in SQL Queries
```sql
...
### 5.3 OVER and PARTITION BY Clause in SQL Queries
```sql
...
### 5.4 Total Aggregation using OVER and PARTITION BY in SQL Queries
```sql
...
### 5.6 Calculate the Average Order Amount per City
```sql
...
### 5.7 Find Top N Customers by Total Spending
```sql
...
### 5.8 Calculate Yearly Order Totals
```sql
...
### 5.9 Rank Customers by Total Order Amount in Mumbai
```sql
...
### 5.10 Compare Each Customer's Total Order to the Average Order Amount
```sql
